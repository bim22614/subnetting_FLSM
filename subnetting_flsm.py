...
def enough_ip():
    ...
    if haved_ip >= needed_ip:
        ...
        print("Not enough ip range for this num of hosts\nEnter more available network")
        start_v()
...
if int(prefix_need[0]) in range(25,33):
    ...
    ip_list_sub.append([[prom_ip_5, prom_ip_4, prom_ip_3, prom_ip_1], [prom_ip_5, prom_ip_4, prom_ip_3, prom_ip_2]])
    for i in range(num_of_sub-1):
        ...
        if prom_ip_1 > 255 or prom_ip_2 > 255:
            ...
            ip_list_sub.append([[prom_ip_5, prom_ip_4, prom_ip_3, prom_ip_1],
...
print("Network address/prefix"+5*' '+"Number of subnets"+5*' '+"Max num of hosts for one sub\n"
      +4*' '+network_addr+'/'+str(prefix)+17*' '+str(num_of_sub)+25*' '+str(max_value_dev))

print()

print("№Net"+5*' '+"Value of hosts"+6*' '+"Network address"+5*' '+"Broadcast address"+8*' '+"Usable hosts")

for i in range(num_of_sub):
    print(str(i+1)+'\t'+10*' '+
          str(max_value_dev)+'\t'+10*' '+
          "{}.{}.{}.{}".format(
              ip_list_sub[i][0][0], ip_list_sub[i][0][1], ip_list_sub[i][0][2], ip_list_sub[i][0][3])+'\t'+7*' '+
          "{}.{}.{}.{}".format(
              ip_list_sub[i][1][0], ip_list_sub[i][1][1], ip_list_sub[i][1][2], ip_list_sub[i][1][3])+'\t'+7*' '+
          "{}.{}.{}.{} - {}.{}.{}.{}".format(
              ip_list_sub[i][0][0],ip_list_sub[i][0][1],ip_list_sub[i][0][2],ip_list_sub[i][0][3]+1,
              ip_list_sub[i][1][0],ip_list_sub[i][1][1],ip_list_sub[i][1][2],ip_list_sub[i][1][3]-1
          ))
